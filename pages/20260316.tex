...
\begin{itemize}
    \item \emph{Linear regression}, to predict continuous responses, \ie, we want to find the distribution of: \begin{equation}
          ...
          \end{enumerate}
    \item \emph{Logistic regression}, to predict binary responses, \ie, we assume that:
          \begin{equation}
...
\begin{proposition*}
    Gaussian and binomial distributions belong to the exponential dispersion family.
\end{proposition*}
              ...
              \begin{equation}
                  \begin{aligned}
                      f(y_i; \mu_i, \sigma^2) & = \frac{1}{\sqrt{2\pi\sigma^2}} \exp\left(-\frac{(y_i - \mu_i)^2}{2\sigma^2}\right)                                          \\
                                              & = \exp\left(-\frac{1}{2}\frac{(y_i - \mu_i)^2}{\sigma^2} - \frac{1}{2}\log(2\pi\sigma^2)\right)                              \\
                                              & = \exp\left(-\frac{1}{2}\frac{y_i^2 - 2\mu_i y_i + \mu_i^2}{\sigma^2} - \frac{1}{2}\log(2\pi\sigma^2)\right)                 \\
                                              & = \exp\left(\frac{y_i\mu_i - \mu_i^2/2}{\sigma^2} - \frac{1}{2}\frac{y_i^2}{\sigma^2} - \frac{1}{2}\log(2\pi\sigma^2)\right)
                  \end{aligned}
              \end{equation}
              and therefore:
              \begin{equation}
                \begin{gathered}
                    \theta_i = \mu_i\quad b(\theta_i) = \frac{\theta_i^2}{2}\quad \phi = \sigma^2\negthinspace,\\
                     a_i(\phi) = \phi\quad c(y_i, \phi) = -\frac{1}{2}\frac{y_i^2}{\sigma^2} - \frac{1}{2}\log(2\pi\sigma^2).
                \end{gathered}
              \end{equation}
        \item[Binomial]
              \begin{equation}
                  \begin{aligned}
                      f(y_i; n_i, \pi_i) & = \binom{n_i}{y_i} \pi_i^{y_i} (1 - \pi_i)^{n_i - y_i}                                                       \\
                                         & = \exp\left(\log\binom{n_i}{y_i} + y_i \log\pi_i + (n_i - y_i)\log(1 - \pi_i)\right)                         \\
                                         & = \exp\left(\log\binom{n_i}{y_i} + y_i \log\pi_i + (n_i - y_i)\log(1-\pi_i)\right)                           \\
              ...
              \begin{equation}
                \begin{gathered}
                    \theta_i = \log\left(\frac{\pi_i}{1 - \pi_i}\right)\quad b(\theta_i) = -n_i \log(1 - \pi_i),\\ 
                    \phi = 1\quad a_i(\phi) = \phi\quad c(y_i, \phi) = \log\binom{n_i}{y_i}.
                \end{gathered}\vspace{-1em}
              \end{equation}
              \qedhere
    \end{description}
\end{proof}
...
\begin{equation}
    \Var(Y_i) = b''(\theta_i)a_i(\phi) = \operatorname{V}(\mu_i)a_i(\phi),
\end{equation}
where $\operatorname{V}(\mu_i)$ is called \emph{variance function}.
...
\begin{enumerate}
    ...
    \item Consider a logistic distribution, which has the following density function:
          ...
          \end{equation}
          We conclude that choosing $f = \bernoulli$ and $g = F^{-1}(z)$, we obtain the logistic regression. If we write explicitly the link function, we have:
          \begin{equation}
              \mu_i = F(\eta_i) = \frac{e^{\eta_i}}{1 + e^{\eta_i}}
          \end{equation}
          and this can be inverted to get:
          \begin{equation}
              \eta_i = g(\mu_i) = F^{-1}(\mu_i) = \log\left(\frac{\mu_i}{1 - \mu_i}\right),
          \end{equation}
          which is called \emph{logit link}.
\end{enumerate}

The second option is also named \emph{canonical link function} and is the default option in the \texttt{glm} function in \texttt{R}. In particular, for this function:
\begin{equation}
    \eta_i = \log\left(\frac{\mu_i}{1 - \mu_i}\right) = \theta_i,
\end{equation}
so $\eta_i = \theta_i$. In general, for a canonical link function:
\begin{equation}
    \xvec_i^\t\beta = \eta_i = g(\mu_i) = \theta_i.
\end{equation}
Since $\mu_i = b'(\theta_i)$, we can also write:
\begin{equation}
    g = (b')^{-1}\negthinspace.
\end{equation}
There are other options for the Bernoulli distribution, such as the \emph{complementary log-log link}, etc.
...
\begin{equation}
    ...
    \end{aligned}
\end{equation}
The \texttt{glm} function in \texttt{R} tries to optimize the log-likelihood function with respect to $\betavec$ using:
\begin{itemize}
