...
\begin{equation}
    \betavech_{\text{LS}} = \argmin_{\betavec} (\Yvec - X\betavec)^\t (\Yvec - X\betavec)\;\longrightarrow\;\betavech_{\text{LS}} = (X^\t X)^{-1} X^\t \Yvec.
\end{equation}

In Ridge regression:
\begin{equation}
    \betavech_{\textsc{Ridge}} = \argmin_{\betavec} (\Yvec - X\betavec)^\t (\Yvec - X\betavec)\quad \text{s.t. } \sum_{j=1}^p \beta_j^2 \leq c, \quad c \geq 0.
\end{equation}
...
\begin{equation}
    \begin{aligned}
        \betavech_{\textsc{Ridge}} & = (X^\t X + \lambda\oone)^{-1} X^\t \Yvec\qquad\expl{$[R = X^\t X]$}                          \\
                                   ...
                                   & = (R + \lambda\oone)^{-1} R \underbracket{(X^\t X)^{-1} X^\t \Yvec}_{\betavech_{\text{LS}}} \\
                                   & = (R(\oone + \lambda R^{-1}))^{-1} R \betavech_{\text{LS}}                                  \\
                                   & = (\oone + \lambda R^{-1})^{-1} \underbracket{R^{-1} R}_{\oone}\betavech_{\text{LS}}        \\
                                   & = {\underbracket{(\oone + \lambda(X^\t X)^{-1})}_{A}}^{\!-1\,} \betavech_{\text{LS}}.
    \end{aligned}
\end{equation}
Therefore, $\betavech_{\textsc{Ridge}} = A \betavech_{\text{LS}}$ where $A$ is a matrix that depends on $\lambda$ and $X\negthinspace$.

We compute the expectation value of $\betavech_{\textsc{Ridge}}$:
\begin{equation}
    \begin{aligned}
        \E[\betavech_{\textsc{Ridge}}] & = A \E[\betavech_{\text{LS}}]                 \\
...
\begin{equation}
    \begin{aligned}
        \text{Bias}(\betavech_{\textsc{Ridge}}) & = \E[\betavech_{\textsc{Ridge}}] - \betavec                            \\
...
\begin{equation}
    \betavech_{\textsc{Lasso}} = \argmin_{\betavec} (\Yvec - X\betavec)^\t (\Yvec - X\betavec)\quad \text{s.t. } \sum_{j=1}^p |\beta_j| \leq c, \quad c \geq 0.
\end{equation}
...
\paragraph{$\bm{\hat{\beta}_{\textsc{Lasso}}}$ vs $\bm{\hat{\beta}_\text{LS}}$}

Consider the least squares estimator $\hat{\beta}_\text{LS}$ for one predictor obtained from:
\begin{equation}
    X^\t(\Yvec - X\beta) = 0.
\end{equation}
...
\begin{itemize}
    \item $\begin{aligned}[t]
                  \beta_{\textsc{Lasso}} > 0 & \iff X^\t(\Yvec - X\beta_\textsc{Lasso}) = \frac{\lambda}{2}                                    \\
                                             & \iff X^\t \Yvec = X^\t X\beta_\textsc{Lasso} + \frac{\lambda}{2}                                \\
                                             & \iff \beta_{\textsc{Lasso}} = (X^\t X)^{-1} X^\t \Yvec - \frac{\lambda}{2} (X^\t X)^{-1}        \\
                                             & \iff \beta_{\textsc{Lasso}} = \beta_\text{LS} - \frac{\lambda}{2} (X^\t X)^{-1}\negthinspace.
              \end{aligned}$
    \item $\begin{aligned}
                  \beta_{\textsc{Lasso}} < 0 & \iff X^\t(\Yvec - X\beta_{\textsc{Lasso}}) = -\frac{\lambda}{2} \iff \beta_{\textsc{Lasso}} = \beta_\text{LS} + \frac{\lambda}{2} (X^\t X)^{-1}\negthinspace.
              \end{aligned}$
    \item $\begin{aligned}[t]
                  \beta_{\textsc{Lasso}} = 0 & \iff X^\t(\Yvec - X\beta_{\textsc{Lasso}}) \in \left[-\frac{\lambda}{2}, \frac{\lambda}{2}\right]                                     \\
                                             & \iff X^\t \Yvec - \underbracket{(X^\t X) \beta_{\textsc{Lasso}}}_{0} \in \left[-\frac{\lambda}{2}, \frac{\lambda}{2}\right]           \\
                                             & \iff \beta_\text{LS} = (X^\t X)^{-1} X^\t \Yvec \in \left[-\frac{\lambda}{2} (X^\t X)^{-1}\negthinspace, \frac{\lambda}{2} (X^\t X)^{-1}\right].
              \end{aligned}$
\end{itemize}
...
\begin{figure}[tb]
    \centering
    \begin{tikzpicture}
        \draw[->] (-3.5,0) -- (3.5,0) node[right] {$\beta_{\text{LS}}$};
        \draw[->] (0,-2.5) -- (0,2.5) node[above] {$\beta_{\textsc{Lasso}}$};
        \draw[thick] (-3.25,-2.5) -- (-.75,0);
        \draw[thick] (.75,0) -- (3.25,2.5);
        \draw[thick] (-.75,0) -- (.75,0);

        \draw[dashed] (-2.5,-2.5) -- (2.5,2.5);

        \node at (-1.25,0) [above] {\scriptsize $-\frac{\lambda}{2} (X^\t X)^{-1}$};
        \node at (1.25,0) [below] {\scriptsize $\frac{\lambda}{2} (X^\t X)^{-1}$};

        \draw[latex-latex] (1.5,.76) -- (1.5,1.49);
        \node[pin=130:{Bias}, font=\small] at (1.5,1.05) {};

        \node[pin=340:{Shrinkage}, font=\small] at (2,1.25) {};
        \node[pin={[pin distance=.75cm]275:{Selection}}, font=\small] at (0,0) {};
    \end{tikzpicture}
    \caption{Soft-thresholding operator.}\label{fig:soft-thresholding-operator}
\end{figure}

\paragraph{Soft-thresholding operator}

Having derived the relation between $\beta_{\textsc{Lasso}}$ and $\beta_\text{LS}$, we can compute $\beta_{\text{LS}}$ (using the closed-form solution) and then add the correction term $\pm \frac{\lambda}{2} (X^\t X)^{-1}$ to get $\beta_{\textsc{Lasso}}$. To do this, we can define the \emph{soft-thresholding operator}, that, for a given $\lambda$, takes $\beta_\text{LS}$ as input and gives $\beta_{\textsc{Lasso}}$ as output.
 In~\Cref{fig:soft-thresholding-operator}, we see that the larger the $\lambda$, the larger the bias; if $\lambda = 0$ there is no bias, and in fact we obtain $\beta_{\textsc{Lasso}} = \beta_\text{LS}$, which is unbiased. Another observation is that for each estimated $\beta_\text{LS}$, if it is smaller than $\frac{\lambda}{2} (X^\t X)^{-1}$ in absolute value, then $\beta_{\textsc{Lasso}}$ is zero, \ie, the Lasso estimator performs variable selection by shrinking some coefficients to zero.

In practice, with many predictors, the Lasso estimator can be computed using \emph{coordinate descent algorithms}, which iteratively update each coefficient while keeping the others fixed.