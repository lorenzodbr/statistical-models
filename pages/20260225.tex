...
\begin{equation}
    \hat{y} = \hat{f}(\xvec).
\end{equation}
Here, $\hat{f}$ is an approximation of some true $f\negthinspace$. \emph{Statistical decision theory} formalizes the type of relationship that we would like to learn.
...
\begin{enumerate}
    \item \emph{Regression setting}:
          ...
          \end{equation}
          where $\tuple{x_i^{(t)}\negmedspace, y_i^{(t)}}$, $i=1,\dots,m$ are testing data points.
...
The $0$-$1$ loss is the simplest loss as it treats every misclassification the same, but there can be differences in loss costs based on the type of error (\eg, false positives vs.\@ false negatives). The $0$-$1$ loss can be generalized to a cost-sensitive loss function:
\begin{equation}
    \begin{array}{|c|c|c|}
        \cline{2-3}
        \multicolumn{1}{c|}{} & \hat{Y} = 0 & \hat{Y} = 1 \\
    ...
    \end{array}
\end{equation}
with $c_0 \neq c_1$. Using the same derivations as before, we get:
\begin{equation}
    \begin{aligned}
