...
\begin{equation}
    \EPE(f) = \E_{\Xvec, Y}[\L(Y\negthinspace, f(\Xvec))] = \int(y - f(\xvec))^2 g(\xvec, y) \odif{\xvec} \odif{y}.
\end{equation}
...
\begin{proof}
    ...
    \begin{equation}
        \begin{aligned}
            \E_{\Xvec, Y}[(Y - f(\Xvec))^2] & = \int (y - f(\xvec))^2 g(\xvec, y) \odif{\xvec} \odif{y}                                                                                                   \\
                                            & = \int \underbrace{(y - f(\xvec))^2 g_{Y\negthinspace,\,\Xvec}(y \given \xvec) \odif{y}}_{\E_{Y\given \Xvec}[(Y - f(\Xvec))^2]} g_{\Xvec}(\xvec) \odif{\xvec} \\
...
\begin{enumerate}
    \item \emph{Linear regression} (parametric), makes three assumptions on $Y$:
          ...
          \begin{equation}
              f(\xvec) = \E[Y \given \Xvec = \xvec] = \beta_0 + \beta_1 x_1 + \ldots + \beta_p x_p.
          \end{equation}
    \item \emph{$k$-nearest neighbours} (non-parametric), makes no assumptions on $Y$ and $f(\xvec)$ is estimated as the average of the $k$ nearest neighbours of $\xvec$:
          \begin{equation}
...
\subsection{Bayes Decision Boundary}
    ...
    \item \emph{Classification setting:} given a $\hat{\P}(1\given \xvec)$ classifier, we can calculate the \emph{confusion matrix}:
          \begin{equation}
              \begin{array}{|c|c|c|}
                  ...
                  \multicolumn{1}{c|}{} & \hat{Y} = 0 & \hat{Y} = 1 \\
                  \hline
                  Y = 0                 & \textrm{TN} & \textrm{FP} \\
                  \hline
                  Y = 1                 & \textrm{FN} & \textrm{TP} \\
                  \hline
              \end{array}
          \end{equation}
          ...
          \begin{itemize}
              \item \emph{True Positive Rate}: \begin{equation}
                        \TPR = \frac{\textrm{TP}}{\textrm{TP} + \textrm{FN}},
                    \end{equation}
              \item \emph{False Positive Rate}: \begin{equation}
                        \FPR = \frac{\textrm{FP}}{\textrm{FP} + \textrm{TN}},
                    \end{equation}
          \end{itemize}
          ...
          \begin{equation}
              \ER = \frac{\textrm{FP} + \textrm{FN}}{\textrm{TP} + \textrm{TN} + \textrm{FP} + \textrm{FN}}.
          \end{equation}
\end{enumerate}
...
If we don't know where to put the threshold to distinguish between classes, we can plot the \acf{roc} curve (\Cref{fig:roc}), which, for varying threshold values, shows the relation between $\TPR$ and $\FPR$. Each point on the curve is associated with a specific pair of $\TPR$ and $\FPR$ values and is derived from a different threshold $t \in [0,1]$. Having the curve, the closer we get to the optimal point ($\TPR = 1$ and $\FPR = 0$), the better the classifier is. In general, one choses the classifier with the largest \acf{auc} or with a curve that is the closes to the optimal point for the range of values of the threshold $(c_0)$ that one is interested in.