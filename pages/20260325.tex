...
In this setting we are dealing with a categorical variable $Y$ and $K$ classes. Na\"ive Bayes belongs to the family of discriminant analysis methods. We recall how to estimate $\P(Y = j \given \Xvec = \xvec)$:
\begin{equation}
    \P(Y = j \given \Xvec = \xvec) = \frac{\pi_j f_j(\xvec)}{\sum_{i = 1}^K \pi_i f_i(\xvec)},\quad j = 1, \dots, K.
\end{equation}
...
A third possible model is the Na\"ive Bayes classifier. The main assumption on $f_i(\xvec)$ is:
\begin{equation}
...
\paragraph{Gaussian Na\"ive Bayes}
...
Therefore, \eqref{assumption:gaussian} and \eqref{assumption:naive} together mean that:
\begin{equation}
    \Sigma_j = \begin{pmatrix}
        \sigma_{j1}^2 & 0             & \cdots & 0             \\
        0             & \sigma_{j2}^2 & \cdots & 0             \\[-1pt]
...
\paragraph{Na\"ive Bayes decision surface}
...
\begin{equation}
    \delta_j(\xvec) = \log(\pi_jf_j(\xvec)) = \log(\pi_j) - \frac{1}{2}\sum_{l = 1}^p \log(\sigma_{\mathit{jl}}^2) - \frac{1}{2}\sum_{l = 1}^p \left(\frac{x_l - \mu_{\mathit{jl}}}{\sigma_{\mathit{jl}}}\right)^2\negthinspace.
\end{equation}
...
\paragraph{Multinomial Na\"ive Bayes classifier}
...
\begin{example}
    ...
    \begin{equation}
        \begin{aligned}
            \theta_{111}   & = \P(X_1 = 1 \given Y = 1)    \\[-5pt]
                           & \quad\vdots                   \\
            \theta_{11G_1} & = \P(X_1 = G_1 \given Y = 1)  \\
            \theta_{121}   & = \P(X_1 = 1 \given Y = 2)    \\[-5pt]
                           & \quad\vdots                   \\
...
\begin{example} Beta-binomial. Consider:
    ...
    \begin{equation}
        \hat{\theta}^{\textsc{(mle)}} = \argmax_{\theta} \L(\theta) = \frac{1}{n}\sum_{i = 1}^n z_i. \expl{(Empirical frequencies)}
    \end{equation}
    ...
    \begin{equation}
        \begin{aligned}
            g(\theta \given \text{data}) & \propto \L(\theta)h(\theta)                                                                                         \\
                                         & \propto \theta^{\sum_{i = 1}^n z_i}(1 - \theta)^{n - \sum_{i = 1}^n z_i}\cdot \theta^{\alpha - 1}(1 - \theta)^{\beta - 1} \\
                                         & = \theta^{\sum_{i = 1}^n z_i + \alpha - 1}(1 - \theta)^{n - \sum_{i = 1}^n z_i + \beta - 1}\negthinspace.
        \end{aligned}
    \end{equation}
    We recognize $g(\theta\given \text{data})$ as a Beta distribution with:
    \begin{equation}
            \alpha^* = \alpha + \sum_{i = 1}^n z_i,\qquad\beta^* = \beta + n - \sum_{i = 1}^n z_i.
    \end{equation}
