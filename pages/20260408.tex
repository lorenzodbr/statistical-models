\section{Adjusted training error}
...
\begin{enumerate}[beginpenalty=10000]
    \item The true model may not be in the family of models we consider (called \emph{misclassification}).
...
\begin{equation}
    Y = \beta_1x_1 + \dots + \beta_px_p + \epsilon = \xvec^\t\betavec + \epsilon,
\end{equation}

We also have a dataset $\tuple{\xvec_i, y_i},\;i = 1, \dots, n$. Our aim is to estimate $\betavec = \tuple{\beta_1, \dots, \beta_p}^\t$ from data. We proceed with a least squares approach. In matrix form we can write:
\begin{equation}
    \begin{aligned}
...
\begin{itemize}
    ...
    \item $\begin{aligned}[t]
                  \Var(\betavech) & = \Var((X^\t X)^{-1} X^\t\yvec)                                                                                  \\
                                  & = (X^\t X)^{-1} X^\t \sigma^2 \oone_p X((X^\t X)^{-1})^\t      \qquad \expl{[$\Var(A\xvec) = A\Var(\xvec)A^\t$]} \\
                                  & = (X^\t X)^{-1} X^\t \sigma^2 \oone_p X(X^\t X)^{-1}    \qquad \expl{$[((X^\t X)^{-1})^\t = (X^\t X)^{-1}]$}     \\
          ...
          \begin{equation}
              \begin{aligned}
                  \Var(\xvec_0^\t\betavech) & = \xvec_0^\t\Var(\betavech)\xvec_0                                                                                                    \\
                                            ...
                                            & = \sigma^2\operatorname{Trace}((X^\t X)^{-1}\xvec_0\xvec_0^\t)  \qquad \expl{[$\operatorname{Trace}(ABC) = \operatorname{Trace}(BC\negthinspace{}A)$]} \\
...
\begin{enumerate}
    \item $n \gg p$: the bias is zero and the variance is low, so the \acs{lr} model will do very well in these cases (if the model is well-specified).
...
If we proceed with the first approach, the idea is to identify a subset of important predictors among $X_1, \ldots, X_p$. \emph{Ideally}, we should consider all possible subsets. The algorithm for selecting the best subset is the following:
\begin{enumerate}
