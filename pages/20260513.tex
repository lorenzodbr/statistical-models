...
\begin{enumerate}
    ...
    \item The variable con also not be time.
...
\begin{equation}
    \underset{\textsc{pdf}}{f(t) = \P(T = t)},\quad\qquad\underset{\textsc{cdf}}{F(t)} = \P(T \leq t),\quad\qquad\underset{\vspace{2pt}\mathclap{\substack{\textsc{survival}\\\textsc{curve}}}}{S(t)} = \P(T > t) = 1 - F(t).
\end{equation}
...
\begin{figure}[ptb]
    ...
    \begin{tikzpicture}[scale=1]
        ...
        \draw[line width=1pt] (0, 1.5) -- (3, 1.5);
        \draw[line width=1pt] (0, 2) -- (4, 2);
        \draw[line width=1pt] (0, 2.5) -- (5, 2.5);

        % Circles at the end of lines
        % Bottom line (y=1) - black circle
        \filldraw[fill=black, draw=black] (1, .5) circle (0.1);
        % Line y=2 - white circle
        \filldraw[fill=white, draw=black] (2, 1) circle (0.1);
        % Line y=3 - black circle
        \filldraw[fill=black, draw=black] (3, 1.5) circle (0.1);
        % Line y=4 - white circle
        \filldraw[fill=white, draw=black] (4, 2) circle (0.1);
        % Line y=5 - black circle
        \filldraw[fill=black, draw=black] (5, 2.5) circle (0.1);
...
\begin{figure}
    ...
    \begin{tikzpicture}
        ...
        \draw[thick] (0, 2) -- (1, 2) -- (1, 1.5) -- (3, 1.5) -- (3, .9) -- (5, .9) -- (5, 0);
...
\begin{equation}
    \L = \prod_{i=1}^n \lambda^{\delta_i} e^{-\lambda y_i} \implies \hat{\lambda} = \argmax_{\lambda} \L
\end{equation}
...
\end{equation}
The curve is therefore typically plotted by fixing all covariates to a level (for example, mean, mode, etc.) and change one variable only ($x_j$) to see how the curve changes with respect to it.

\begin{note}
    The exponential may be too restrictive in many applications. Weibull distribution (two parameters) is quite common in engineering (reliability modelling).
\end{note}