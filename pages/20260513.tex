...
\begin{equation}
    \begin{aligned}
        \qquad f(t) &= \P(T = t),\qquad\qquad\qquad\,\,\expl{(PDF)}\\
        \qquad F(t) &= \P(T \leq t),\qquad\qquad\qquad\,\,\expl{(CDF)}\\
        \qquad S(t) &= \P(T > t) = 1 - F(t).\qquad\expl{(Survival function)}
    \end{aligned}
\end{equation}
...
\end{equation}
since there is no data point greater than $y_5$. Therefore, the survival curve is given by the step function showed in~\Cref{fig:survival-curve}.
\begin{figure}
    \centering
    \begin{tikzpicture}
...
is estimated using:
\begin{equation}
    \hat{\P}(T > d_j \given T > d_{j-1}) = 1 - \frac{q_j}{r_j} = \frac{r_j - q_j}{r_j},
\end{equation}
so that:
\begin{equation}
...
\begin{equation}
    \L(\lambda) = \prod_{i=1}^n \lambda^{\delta_i} e^{-\lambda y_i} \implies \hat{\lambda} = \argmax_{\lambda} \L(\lambda)
\end{equation}
...
\end{equation}
Therefore, the curve is typically plotted by fixing all covariates to a level (for example, mean, mode, etc.) and changing one variable only ($x_j$) to see how the curve changes with respect to it.
