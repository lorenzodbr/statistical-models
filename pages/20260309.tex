...
\begin{enumerate}
    ...
    \item \emph{Linear regression}:
          \begin{equation}
              \widehat{\E[Y \given \Xvec = \xvec]} = \hat{\beta}_0 + \hat{\beta}_1 x_1 + \cdots + \hat{\beta}_p x_p = \beta_0 + \xvec^\t \hat{\betavec}.
          \end{equation}
\end{enumerate}
...
\begin{enumerate}
    ...
    \item Estimate the following quantity:
          \begin{equation}
              \widehat{\P(Y = j \given \Xvec = \xvec_0)} = \frac{1}{k} \sum_{i\,\in\,N_k(\xvec_0)}\hspace{-7pt}\oone(y_i = j), \quad j = 1, \ldots, C,
          \end{equation}
...
\begin{examples}\leavevmode
    \begin{enumerate}
        ...
        \item Let us now consider only a binary response:
              ...
              \end{equation}
              which is not possible since the left-hand side is a probability and thus must be between $0$ and $1$.\qedhere
    \end{enumerate}
\end{examples}
...
\begin{figure}[tb]
    ...
    \begin{tikzpicture}
        \begin{axis}[
            ...
            \addplot[domain=-7:7, samples=100, thick] {exp(x) / (1 + exp(x))};
            \addplot[domain=-7:7, samples=100, thick, dashed] {exp(-x) / (1 + exp(-x))};
...
\begin{example}
    ...
    \begin{equation}
        \P(1 \given \xvec) = \frac{e^{\beta_0 + \beta_1 x_1}}{1 + e^{\beta_0 + \beta_1 x_1}}.\qedhere
    \end{equation}
\end{example}
...
\begin{enumerate}
    \item The estimated coefficients of the model: $\hat{\beta}_0, \hat{\beta}_1, \ldots, \hat{\beta}_p$.
