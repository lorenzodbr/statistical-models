\chapter{Optimizing models}

\section{Resampling methods}

We have seen a number of classifiers, such as \acs{lr}, \acs{lda}, \acs{qda}, Na\"ive Bayes, $k$-NN. In this chapter, we will see how to choose the optimal model among a set of candidate models. We will also see how to estimate the test error rate of a model.
...
\paragraph{Resampling methods} The general idea is to hold out a subset of the data from the fitting process, so that we can use these observations to test the fitted model. There are various (simpler and more complex) versions of this idea, that try to reach a compromise between:
\begin{enumerate}
...
\begin{enumerate}[beginpenalty=10000]
    \item \emph{Validation set approach.} We randomly divide the dataset into two parts: training set and validation set. Typical split proportions are $50$-$50$ and $70$-$30$. Then, the model is fitted on training data and tested on validation data.
          ...
          \begin{itemize}
              ...
              \item Not using all the data to fit and all the data to test the model.
          \end{itemize}
    ...
    \item \emph{$k$-fold cross-validation.} With this approach we randomly divide data into $k$ equally sized parts (typical values are $k = 5$ or $k = 10$). Then:
          ...
          Formally:
          \begin{enumerate}
              \item Fix a model (\ie, set parameters, $d$, \etc).
              \item Let $C_1, \dots, C_k$ be the sets of indices of the observations in each fold. For each fold $C_i$, calculate the error after having fitted the model on the remaining $k - 1$ folds:
                    \begin{equation}
                        E_i = \begin{dcases}
                            \acs{mse}_i\colon \frac{1}{\abs{C_i}}\sum_{j\,\in\,C_i} (y_j - \hat{y}_j)^2      & \text{if regression},     \\
                            \acs{er}_i\colon \frac{1}{\abs{C_i}}\sum_{j\,\in\,C_i} \oone(y_j \neq \hat{y}_j) & \text{if classification},
...
\begin{example}
    Let's compare $k$-NN and logistic regression in a setting with $X_1, \ldots, X_p$ predictors. If we perform \acs{cv} on full data and vary the parameter $k$ we obtain two curves, one for $k$-NN and one for logistic regression (the latter will be constant as it doesn't depend on $k$, as shown in~\Cref{fig:cv_comparison}). Since we chose the optimal $k$ for $k$-NN, and at $\hat{k}$ the error for $k$-NN is lower than the error for logistic regression, we might chose the former. This comparison, however, is a bit unfair towards logistic regression since the \acs{cv} procedure to select $k$ has seen the full data.
\end{example}

To formalize: even if both are called ``\acs{cv} error''\negthinspace, two models may not be comparable because:
\begin{itemize}
    \item They may use different \acs{cv} splits or folds.
    \item The random seed may differ, which leads to different partitions.
    \item Models may have different tuning procedures embedded in \acs{cv}.
    \item Each \acs{cv} error is conditional on its own model selection process.
\end{itemize}
So, the two estimates are not guaranteed to be evaluated on the same test folds or same resampling scheme and this makes the comparison unfair and potentially misleading.

\begin{figure}
    \centering
    \begin{tikzpicture}
        \draw[->] (0, 0) -- (5, 0) node[right] {$k$};
        \draw[->] (0, 0) -- (0, 3) node[above] {\acs{cv} error};
        \draw[thick, densely dashed] (0, 1.5) -- (5, 1.5) node[right] {\small\acs{lr}};
        \draw[thick] (.25, 2.5) parabola bend (2.5, 0.5) (4.75, 2.5) node[above right] {\small $k$-NN};
        \draw[semithick, dotted] (2.5, 0) node[below] {$\hat{k}$} -- (2.5, 1.5);
        \fill (2.5, 0.5) circle (0.07);
        \fill (2.5, 1.5) circle (0.07);
    \end{tikzpicture}
    \caption{Comparison of $k$-NN and logistic regression.}
