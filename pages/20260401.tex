...
\begin{enumerate}[beginpenalty=10000]
    ...
    \item \emph{$k$-fold cross-validation.} With this approach we randomly divide data into $k$ equally sized parts (typical values are $k = 5$ or $k = 10$). Then:
          ...
          \begin{enumerate}
              ...
              \item Let $C_1, \dots, C_k$ be the indices of the observations in each fold. For each fold $C_i$, calculate the error:
                    \begin{equation}
                        E_i = \begin{dcases}
...
To formalize: even if both are called ``CV error''\negthinspace, two models may not be comparable because:
\begin{itemize}
    \item They may use different CV splits or folds.
