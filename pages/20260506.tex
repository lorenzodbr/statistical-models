    ...
    \begin{equation}
        \begin{aligned}
            K(\xvec, \zvec) & = (1 + \xvec^\t\zvec)^2                                                                  \\
                            & = 1 + 2\sum_{j=1}^p x_j z_j + \sum_{j,j=1}^p x_jz_j\cdot x_k z_k                         \\
                            & = 1 + \sum_{j=1}^p \sqrt{2}x_j \cdot \sqrt{2}z_j + \sum_{j,k=1}^p x_j x_k \cdot z_j z_k.
        \end{aligned}
    \end{equation}
    We can define $\Phi\colon \rr^p \to \rr^N$ as:
    \begin{equation}
        \Phi(\xvec) = \tuple{1, \sqrt{2}x_1, \dots, \sqrt{2}x_p, x_1^2, \dots, x_p^2, \sqrt{2}x_1x_2, \dots, \sqrt{2}x_{p-1}x_p} \in \rr^{1+p+p^2}.\qedhere
    \end{equation}
\end{example}
...
\end{equation}
the only difference is the loss function. The loss function of \acsp{svc} is called \emph{hinge loss}, while the one of Ridge regression is just the \emph{squared error loss}. So, \acsp{svm} use a different loss function:
\begin{equation}
