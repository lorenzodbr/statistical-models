...
\begin{equation}
    f(\xvec) = \beta_0 + {\underbracket{\betavec}_{\mathclap{\betavec\,\in\,\rr^N}}}^\t \underbracket{\Phi(\xvec)}_{\zvec} = \beta_0 + \sum_{i = 1}^n \underbracket{\gamma_i}_{\gammavec\,\in\,\rr^n} \negthickspace y_i \Phi(\xvec_i) \cdot \Phi(\xvec).
\end{equation}
...
\begin{figure}[tb]
    ...
    \begin{tikzpicture}
        \begin{axis}[
            ...
            \addplot[domain=-1.75:1.75, samples=1000, thick] {x <= 1 ? 1 - x : 0};
        \end{axis}
    \end{tikzpicture}
...
\begin{figure}
    ...
    \begin{tikzpicture}
        \begin{axis}[
            ...
            \addplot[domain=-2.75:2.75, samples=100, thick] {max(0, abs(x) - 1)};
        \end{axis}
    \end{tikzpicture}
