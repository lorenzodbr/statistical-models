...
\begin{equation}
    f(\xvec) = \beta_0 + {\underbracket{\betavec}_{\mathclap{\betavec \in \rr^N}}}^\t \underbracket{\Phi(\xvec)}_{\zvec} = \beta_0 + \sum_{i = 1}^n \underbracket{\gamma_i}_{\gamma \in \rr^n} \negthickspace y_i \Phi(\xvec_i) \cdot \Phi(\xvec).
\end{equation}
...
The loss function of \acsp{svc} is called \emph{hinge loss}, while the one of logistic regression is just the \emph{squared error loss}. So, \acsp{svm} use a different loss function:
\begin{equation}
