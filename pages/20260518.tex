...
\begin{note}
    ...
    Since $h(0)$ can be flexible, this is a less restrictive model. Moreover, since we have a multiplicative factor, two survival curves for different covariate values cannot cross.
\end{note}
...
\end{equation}
as a measure of the predictive performance of the model, with $\betavech$ estimated from training data. We multiply by $\delta_{i^*}$ to select only those $i^*$ that are not censored.
...
\end{equation}
where $\xvec_{s,r}$ is a vector of covariates associated to the pair $(s,r)$ at time $t$. The aim is to infere $\betavec$ from data.
...
\begin{examples}
    ...
    \begin{itemize}
        ...
        \item Triadic (transitive) closure:
              \begin{equation}
                  x_{\mathrm{sr}}(t) = \begin{cases}
                      1 & \text{if } \tuple{s, k} \text{ and } \tuple{k, r} \text{ happened before time } t, \\
                      0 & \text{otherwise}.
                  \end{cases}
              \end{equation}
    \end{itemize}
    The first two are called \emph{dyadic}. The first three are exogenous, while the last two are endogenous.
\end{examples}

Let's analyize the partial likelihood in this context. We end up with a similar expression as before:
\begin{equation}
