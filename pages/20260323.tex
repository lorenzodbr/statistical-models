...
\subsection{\texorpdfstring{\acs{lda} with $p \geq 1$}{LDA with p >= 1}}
...
\begin{enumerate}
    ...
    \item $\Sigma_1 = \Sigma_2 = \ldots = \Sigma_K = \Sigma = \begin{pmatrix}
              ...
              \end{pmatrix},$ where $\sigma_{jl} = \Cov(X_j, X_l\given Y = k)$ and $\sigma^2_j = \Var(X_j\given Y = k)$.
\end{enumerate}
...
\begin{example}
    For example, with $K = 2, p = 50$, \acs{lda} has $1375$ parameters, while \acs{qda} has $2650$ parameters. Any choice between the two models need to involve that data.
\end{example}
...
\begin{itemize}
    ...
    \item \acs{lda} is more restrictive as it makes the assumption that $\Xvec$ is a multivariate Gaussian conditional on the class, so this needs to be checked.
