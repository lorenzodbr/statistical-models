...
\begin{equation}
    \begin{aligned}
        \tilde{C}_2\colon & \frac{1}{\norm{\betavec}}y_i(\beta_0 + \betavec^\t\xvec_i) \geq M(1 - \epsilon_i), \quad i = 1, \dots, n, \\
        \iff              & y_i(\beta_0 + \betavec^\t\xvec_i) \geq \norm{\betavec}M(1 - \epsilon_i), \quad i = 1, \dots, n.
    \end{aligned}
\end{equation}
...
\begin{equation}
    \begin{aligned}
         & \max_{\beta_0, \betavec, \epsilonvec} \frac{1}{\norm{\betavec}}                             \\
         ...
         & \min_{\beta_0, \betavec, \epsilonvec} \norm{\betavec}^2                                     \\
         & \text{s.t.}\; y_i(\beta_0 + \betavec^\t\xvec_i) \geq 1 - \epsilon_i, \quad i = 1, \dots, n, \\
         & \phantom{s.t.} \epsilon_i \geq 0, \quad i = 1, \dots, n,                                    \\
         & \phantom{s.t.} \sum_{i=1}^n \epsilon_i \leq C.
    \end{aligned}
...
\begin{equation}
    \begin{aligned}
         ...
         & \text{s.t.}\; g_i(\alphavec) = 0, \quad i = 1, \dots, d,     \\
         & \phantom{s.t.} h_j(\alphavec) \leq 0, \quad j = 1, \dots, m.
    \end{aligned}
\end{equation}
...
\begin{equation}
    \begin{aligned}
        \min_{\alphavec} f(\alphavec) & = \min_{\alphavec} \theta_\textsc{p}(\alphavec)         \\
...
The $p$ in $\theta_\textsc{p}$ stands for \emph{primal}, since we are still considering the original optimization problem. We can also define the \emph{dual} version of the problem as:
\begin{equation}
...
\paragraph{Back to Support Vector Classifiers} Now, we need to rewrite the constraints in order to apply the KKT conditions. We have:
\begin{equation}
    \begin{aligned}
...
\end{itemize}
On top of that, many $\gamma_i$ will be zero. In fact, using the KKT conditions, $\delta_i^*g_i(\alphavec^*) = 0$:
