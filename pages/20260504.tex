...
\paragraph{Optimization with equality and inequality constraints} Consider the general problem:
\begin{equation}
    \begin{aligned}
...
\paragraph{Karush-Kuhn-Tucker conditions} Consider the optimal $\alphavec^*$\negthinspace, $\gammavec^*$ and $\deltavec^*$ and $\theta_{\textsc{p}}(\alphavec^*) = \theta_{\textsc{d}}(\gammavec^*\negthinspace, \deltavec^*)$. Then:
\begin{equation}
...
\begin{proof}
    \begin{equation}
        \begin{aligned}
            f(\alphavec^*) = \min_{\alphavec} \lagr(\alphavec, \gammavec^*\negmedspace, \deltavec^*) & = \min_{\alphavec} \left(f(\alphavec) + \sum_{i=1}^d \gamma_i^* h_i(\alphavec) + \sum_{j=1}^m \delta_j^* g_j(\alphavec)\right)                         \\
                                                                                                    & = f(\alphavec^*) + \underbracket{\sum_{i=1}^d \gamma_i^* h_i(\alphavec^*)}_{0} + \underbracket{\sum_{j=1}^m \delta_j^* g_j(\alphavec^*)}_{\leq 0} \\
                                                                                                    & \leq f(\alphavec^*).
        \end{aligned}
    \end{equation}
    This implies:
    \begin{equation}
        \sum_{i = 1}^m \underbracket{\delta_i^* g_i(\alphavec^*)}_{\leq 0} = 0 \implies \delta_i^* g_i(\alphavec^*) = 0, \quad i = 1, \dots, m.\qedhere
    \end{equation}
\end{proof}
...
\end{align}
If we plug these values back into the dual formulation, we get:
\begin{equation}
    \begin{aligned}
        \theta_{\textsc{d}}(\gammavec, \deltavec, \mu) & = \frac{1}{2}\underbracket{\betavec^\t\betavec}_{\norm{\betavec}} - \sum_{i=1}^n (\underbracket{\gamma_i y_i}_{0}\beta_0 + \gamma_iy_i\betavec^\t\xvec_i - \gamma_i + \underbracket{\epsilon_i\gamma_i) - \sum_{i=1}^n \delta_i\epsilon_i + \mu\sum_{i=1}^n \epsilon_i}_{-\gamma_i - \delta_i + \mu = 0} - \mu C \\
...
\begin{equation}
    \begin{aligned}
         & \max_{\gammavec\negthinspace,\,\mu} \sum_{i=1}^n \gamma_i - \frac{1}{2}\sum_{i,j} \gamma_i\gamma_j y_i y_j \xvec_i\cdot\xvec_j - \mu C, \\
