...
\begin{definition*}[Hyperplane]
    In a $p$-dimensional space, a \emph{hyperplane} is a flat affine subspace of dimension $p-1$.
\end{definition*}
...
Given the data:
\begin{equation}
    X_{n \times p}\quad\xvec_1, \ldots, \xvec_n
\end{equation}
...
\begin{equation}
    y_i(\beta_0 + \betavec^\t\xvec_i) \geq 0, \quad i = 1, \ldots, n.
\end{equation}
Such a hyperplane can be used for classification for a new $\xvec^*$:
\begin{equation}
    f(\xvec^*) = \beta_0 + \betavec^\t\xvec^* \mapsto y^* = \begin{cases}
        1  & \text{if } f(\xvec^*) > 0, \\
        -1 & \text{if } f(\xvec^*) < 0.
    \end{cases}
\end{equation}
...
A hyperplane is defined by its orthogonal direction given by $\betavec$. Let us consider only $\betavec$s such that $\norm{\betavec} = \sqrt{\beta_1^2 + \dots + \beta_p^2} = 1$. The distance of a point $\xvec_i$ to the hyperplane is obtained considering a generic point $\zvec$ on the hyperplane, \ie, such that $\beta_0 + \betavec^\t\zvec = 0$, and projecting the vector $\xvec_i - \zvec$ on the direction $\betavec$:
\begin{equation}
...
\begin{equation}
    \begin{aligned}
        \tuple{\hat{\beta}_0, \betavech} = & \;\argmax_{\beta_0, \betavec} \min_{i = 1, \ldots, n} y_i(\beta_0 + \betavec^\t\xvec_i) \\
                                           & \;\text{s.t.}\; \norm{\betavec} = 1.
    \end{aligned}
\end{equation}
...
\begin{itemize}
    ...
    \item In general, there are not many observations that are the closest to the hyperplane. They are called \emph{support vectors} and completely characterize the hyperplane.
\end{itemize}

However, there are some problems with the maximal margin classifier:
\begin{enumerate}
...
\begin{itemize}
    ...
    \item $0 < \epsilon_i \leq 1$ for observations on the correct side of the hyperplane but less than $M$ away from it;
