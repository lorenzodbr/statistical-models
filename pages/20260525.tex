...
Formally, we split data $D$ into meaningful (homogeneous) clusters, $C_1, C_2, \dots, C_K$, that form a partition, \ie:
\begin{equation}
    \bigcup_{k=1}^K C_k = D\negthinspace, \qquad C_i \cap C_j = \emptyset, \quad i \neq j.
\end{equation}
We can characterize homogeneity in different ways, such as:
\begin{itemize}
...
In general we want:
\begin{equation}
    \tuple{C_1, \ldots, C_K} = \negthickspace\argmin_{\substack{\tuple{\Gamma_1, \ldots, \Gamma_K} \\ \text{partition of } D}}\negmedspace\sum_{i=1}^K W(\Gamma_i),
\end{equation}
...
\end{equation}
The $\bar{x}_{\mathit{il}}$ is the sample mean of predictor $l$ in cluster $\Gamma_i$.
    ...
    \begin{equation}
        \begin{aligned}
            2W(\Gamma_i) & = \frac{1}{\abs{\Gamma_i}} \sum_{x, y\,\in\,\Gamma_i} (x - y)^2                                                                                                                                                   \\
                         ...
                         & = 2\left(\sum_{x\,\in\,\Gamma_i} x^2 - \sum_{x\,\in\,\Gamma_i}2\bar{x}_i x + \sum_{x\,\in\,\Gamma_i}\bar{x}_i^2\right)\\
...
\begin{figure}[tb]
    ...
    \begin{forest}  LT
        ...
        \draw[latex-latex] (-0.6, -3.7) -- (-0.6, -2.3) node[pos=0.69, right] {\scriptsize $d_2$};
        \draw[latex-latex] (-1.15, -3.7) -- (-1.15, -1.3) node[pos=0.735, right] {\scriptsize $d_1$};
        \draw[dashed] (-3, -.65) -- (3, -.65) node[pos=0.85, above] {\scriptsize $K = 2$};
        \draw[dashed] (-3, -1.65) -- (3, -1.65) node[pos=0.875, above] {\scriptsize $K = 4$};
    \end{forest}
    \caption{Example of a dendogram. The three leaves $a$, $b$ and $c$ are observations.}
    \label{fig:dendogram}
\end{figure}
...
\begin{description}[beginpenalty=10000]
    ...
    \item[Hierarchical clustering] It is a family of clustering techniques with various distance measures that can be used. Differently to $K$\negthinspace-means, $K$ is not fixed a priori. The output is a hierarchy of nested clusters, with the hierarchy represented as a \emph{dendogram} (\Cref{fig:dendogram}). In the figure, similarity is pictured vertically and not horizontally, in the sense that the observations $b$ and $c$ are ``closer'' to each other than $a$ and $b$, because the split happened at a higher level of the hierarchy (\ie, $d_1 > d_2$).
          ...
          In addition to the distance between the observations, the method needs a distance between clusters in order to merge/split them. This is called \emph{linkage criterion} and a common choice for it is the \emph{average linkage}:
          \begin{equation}
          ...
          \begin{itemize}
              \item \emph{Agglomerative}: start with each observation in one cluster and then merge the two clusters that are closest to each other; continue until a stopping criterion is met.
...
\paragraph{Selection of $\bm{K}$} To select $K$ for the $K$\negthinspace-means algorithm, we can use the \emph{elbow method}, which consists in plotting the optimality criterion as a function of $K$ and looking for an elbow in the plot (\Cref{fig:elbow_method}). The idea is that, as $K$ increases, the optimality criterion will decrease, but after a certain point the improvement will be negligible. Therefore, we look for the point where the improvement starts to be negligible, which is exactly the elbow of the plot.
...
\subsection{Clustering network data}
...
\paragraph{Optimality criterion} To define an optimality criterion we need to understand what groups have in common. Let $\bm{\ell} = \tuple{C_1, \dots, C_K}$ be a partition of the nodes and $f_{\mathit{ij}}(\bm{\ell})$ the fraction of edges in the network that connect a node in $C_i$ to a node in $C_j$ with respect to the total number of edges. In particular, $f_{\mathit{ii}}(\bm{\ell})$ is the fraction of edges in cluster $C_i$. We can define the following optimality criterion (\emph{modularity}) as:
\begin{equation}
    M(\bm{\ell}) = \sum_{i=1}^K (f_{\mathit{ii}}(\bm{\ell}) - f_{\mathit{ii}}^*)^2\negthinspace,
\end{equation}
where $f_{\mathit{ii}}^*$ is the expected value of $f_{\mathit{ii}}(\bm{\ell})$ under a random assignment of edges to nodes. Large values of modularity suggest \emph{non-trivial} (\ie, not random) group structures.
