...
\begin{enumerate}
    ...
    \item \emph{\acl{lda}} (\acs{lda}, Fisher, 1936), which provides a qualitative response. It can be extended to \emph{\acf{qda}} and applied to Na\"ive Bayes classifiers.
    \item \emph{Logistic Regression} (1940s)
    \item \emph{\aclp{glm}} (\acsp{glm}, 1970s)
    \item \emph{\aclp{crt}} (\acsp{crt}, 1980s), which are computer-intensive methods and can be extended to \emph{Random Forests} and \emph{Boosting}.
...
\begin{example}
    ...
    \begin{itemize}
        ...
        \item Then, $Y = f(\Xvec) + \epsilon$ is our prediction. The $f$ is a fixed but unknown function of $X_1, \dots, X_p$ (systematic component) and $\epsilon$ is a stochastic component such that:
            ...
            \end{equation}
            The perpendicular symbol means that $\epsilon$ is independent of $\Xvec$, so the error does not depend on the predictors.\qedhere
    \end{itemize}
\end{example}

The estimation of $f$ is important for two reasons:
\begin{description}
    \item[Prediction] We want to predict $Y$ when we only have observations of $\Xvec$. Given $\hat{f}$ estimate of $f\negthinspace$, the prediction of $Y$ when $\Xvec = \xvec$ is $\hat{Y} = \hat{f}(\xvec)$. If this is the only goal, then $\hat{f}$ can also be a black-box, \ie, we are not interested in the relationship between $Y$ and $\Xvec$, but only in the accuracy of the prediction.
    ...
    \item[Inference] We want to answer to different questions:
          ...
          \begin{itemize}
              \item \emph{Parametric approaches}: we make an assumption about the functional form of $f\negthinspace$, which depends on parameters, \eg:
                    \begin{equation}
